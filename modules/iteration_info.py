...
headers = {
    ...
    "Authorization": f"Basic {os.environ.get('AZDO_AUTH')}"  # fallback if you prefer env
}

# If not using env var, use PAT directly
import base64
pat_bytes = f":{personal_access_token}".encode("utf-8")
headers["Authorization"] = "Basic " + base64.b64encode(pat_bytes).decode("utf-8")
...
def get_work_items_for_iteration(iteration_path):
    """Get all work items for a given iteration path."""
    wiql = {
        "query": f"""
...
def refresh_iterations():
    """Fetch iteration data from ADO and store in MongoDB."""
    ...
    print("Iteration data refreshed and stored in MongoDB.")
